...
from app.auth.jwt_handler import creat_jwt
...
def authentificate_user(username:str, password:str, db:db):
    user_db = db.query(Users).filter(Users.username == username).first()
    if not user_db :
...
async def register_user(user: UsersLogin, db: db):
...
@router.post("token", summary="Connexion", description="Utilise ton nom d'utilisareur et mot de passe pour te connecter", response_model=Token)
